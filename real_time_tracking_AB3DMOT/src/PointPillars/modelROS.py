...
class AB3DMOT():			  	
	def __init__(self,  ID_init=0, pointcloud_topic=None):
	...
	def callbackFunction(self, lidarMsg, imuMsg, gpsFixMsg, gpsVelMsg):
		...
		try:
			pc_np = self.pointcloud2_to_xyz(lidarMsg)

			self.pc_filtered = self.point_range_filter(pc_np)
			result = self.process_point_cloud(self.pc_filtered)
			pcd_limit_range = np.array([0, -40, -3, 70.4, 40, 0.0], dtype=np.float32)
			result_filter = keep_bbox_from_lidar_range(result, pcd_limit_range)

			box = result_filter['lidar_bboxes']
			label, scores = result_filter['labels'], result_filter['scores']
			...
			for i, trk in enumerate(trk_result):
				...
				if trk[9] > 0.5:
					...
					marker.scale.x, marker.scale.y, marker.scale.z = 0.8, 0.8, 2.8
					...
					# 바운딩 박스 마커 설정
					bbox_marker.header = lidarMsg.header
					bbox_marker.type = marker.CUBE
					bbox_marker.id = int(trk[7]) + 1000  # 텍스트 마커와 ID 충돌 방지
					bbox_marker.action = marker.ADD
					bbox_marker.frame_locked = True
					bbox_marker.lifetime = rospy.Duration(0.1)
					bbox_marker.scale.x = trk[2]
					bbox_marker.scale.y = trk[1]
					bbox_marker.scale.z = trk[0]
					bbox_marker.color.r = 0.0
					bbox_marker.color.g = 1.0
					bbox_marker.color.b = 0.0
					bbox_marker.color.a = 0.5
					bbox_marker.pose.position.x = trk[3]
					bbox_marker.pose.position.y = trk[4]
					bbox_marker.pose.position.z = trk[5]
					...
					# BoundingBoxArray 설정
					bbox.header = lidarMsg.header
					bbox.pose.position.x = trk[3]
					bbox.pose.position.y = trk[4]
					bbox.pose.position.z = trk[5]
					bbox.pose.orientation.w = q[3]
					bbox.pose.orientation.x = q[0]
					bbox.pose.orientation.y = q[1]
					bbox.pose.orientation.z = q[2]
					bbox.dimensions.x = trk[2]
					bbox.dimensions.y = trk[1]
					bbox.dimensions.z = trk[0]
					bbox.value = trk[9]
					...
					self.markerArray.markers.append(bbox_marker)
...
if __name__ == '__main__':
	...
	ID_start=1
	AB3DMOT( ID_init=ID_start, pointcloud_topic=topic)
