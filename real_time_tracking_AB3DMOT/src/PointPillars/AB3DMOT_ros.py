...
class AB3DMOT():
    def __init__(self, ID_init=0, model=None, pointcloud_topic=None):
        ...
        ds = message_filters.ApproximateTimeSynchronizer(([self.pcdSub, self.imuSub, self.gpsFixSub, self.gpsVelSub]),
                                                         1, 0.1)
    ...
    def crop_pointcloud(self, pointcloud):
        ...
        mask = np.where(
            (pointcloud[:, 0] >= lim_x[0]) & (pointcloud[:, 0] <= lim_x[1]) & (pointcloud[:, 1] >= lim_y[0]) & (
                        pointcloud[:, 1] <= lim_y[1]) & (pointcloud[:, 2] >= lim_z[0]) & (pointcloud[:, 2] <= lim_z[1]))
    ...
    def callbackFunction(self, lidarMsg, imuMsg, gpsFixMsg, gpsVelMsg):
        ...
        roll, pitch, yaw = euler_from_quaternion(imuMsg.orientation.x, imuMsg.orientation.y, imuMsg.orientation.z,
        ...
        oxtsdata = OxtsPacket(lat, lon, alt, roll, pitch, yaw, vn, ve, vf, vl, vu
                              , ax, ay, az, af, al, au, wx, wy, wz, wf, wl, wu, pos_accuracy, vel_accuracy, navstat,
        ...
        for i, trk in enumerate(trk_result):
            ...
            if trk[9] > 0.5:
                ...
                bbox.pose.orientation.w, bbox.pose.orientation.x, bbox.pose.orientation.y, bbox.pose.orientation.z = q[
                    3], q[0], q[1], q[2]
